...
final class SwingPopupMenuPeer extends SwingBaseMenuPeer<PopupMenu, JPopupMenu>
    ...
    public SwingPopupMenuPeer(SwingToolkit toolkit, PopupMenu popupMenu) {
        ...
        for (int i = 0; i < item_count; i++) {
            ...
            menu_item.addNotify();
          //TODO jComponent.add(((SwingMenuComponentPeer<?, ?>) menu_item.getPeer()).jComponent);
        }
    }
    ...
    public void show(Component component, int x, int y) {
    	//TODO ((JPopupMenu) jComponent).show(((SwingComponentPeer<?, ?>) component.getPeer()).peerComponent, x, y);
    }

    @SuppressWarnings("deprecation")
    public void addItem(MenuItem item) {
        item.addNotify();
      //TODO jComponent.add(((SwingBaseMenuItemPeer<?, ?>) item.getPeer()).jComponent);
    }
